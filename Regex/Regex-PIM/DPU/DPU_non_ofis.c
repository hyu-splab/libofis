...
// MRAM variables
__mram_noinit int32_t MRAM_DFA_set[DFA_SET_MAX_SIZE]; // DFA info transferred from Host
__mram_noinit int8_t MRAM_target_text[1 << 19]; // Text info
__mram_noinit int32_t MRAM_output_buffer[(1 << 20) + 1024]; // matching result
__mram_noinit int32_t MRAM_output_metadata[2]; // metadata of matching result. (#matching * 2) save into 0-index.
...
__dma_aligned int8_t partitioned_data[2][2048]; // buffer for saving partitioned target data
__dma_aligned int32_t partitioned_chunk_num[2]; // save chunk number allocated to each buffer
...
__dma_aligned int32_t global_matching_index; // shared variable for index accesing
...
void chunk_replace(int chunk_num) // chunk exchange
{
    ...
    mram_read( (const __mram_ptr void*) (MRAM_target_text + 2048 * chunk_num), (void *) (partitioned_data[(chunk_num) % 2]), 2048); // chunk exchange
    partitioned_chunk_num[chunk_num % 2] = chunk_num; // update chunk number
}


int get_DFA_info() //  data extraction from DFA_set
{
    // fixed_data_copy
    ...
    if(states_num == -99 && character_set_size == -99) return 1; // end of DFA_set
    ...
    // bitmap for 32-state's acceptability
    int32_t bitmap_size;
    bitmap_size = ((states_num + 31) >> 5); // same role with ceil(states_num / 32)
    if(bitmap_size & 1) bitmap_size++; // bitmap's size must be even
    acceptable_state_bitmap = (int32_t *) mem_alloc(4 * bitmap_size); // bitmap dynamic allocation
    mram_read( (const __mram_ptr void*) (MRAM_DFA_set + current_DFA_index), (void *) acceptable_state_bitmap, 4 * bitmap_size); // copy bitmap
...
int matching(/*int start_index, int start_state*/int chunk_num, int start_index, int start_state, int tasklet_id)
{
    ...
    for(int i = start_index; i < 2048; i++)
    {
        ...
        //     printf("state: %d\n", current_state);
        // }
        if(current_state == 0) // if current state is trap state
        {
            return 0;
        }
