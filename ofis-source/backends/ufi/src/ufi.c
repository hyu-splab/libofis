				     ...
				     u16 *pcs)
{
	u32 status;
	u8 nr_cis = GET_DESC_HW(rank)->topology.nr_of_control_interfaces;
	u8 nr_bits_in_lsb = GET_DESC_HW(rank)->memories.ci_pc_lsb_nr_phys_bits;
...
__API_SYMBOL__ u32 ufi_exec_write_structure(struct dpu_rank_t *rank, u8 ci_mask,
					    u64 structure)
{
	u32 status = DPU_OK;
	u64 *cmds = GET_CMDS(rank);
...
__API_SYMBOL__ u32 ufi_exec_void_frame(struct dpu_rank_t *rank, u8 ci_mask,
...
__API_SYMBOL__ u32 ufi_exec_8bit_frame(struct dpu_rank_t *rank, u8 ci_mask,
