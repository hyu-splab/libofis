    ...
    do
        ...
        do
            if [ $file_type -eq 0 ]; then
                sudo LD_LIBRARY_PATH=$LD_LIBRARY_PATH ./bin/CSR_host -f"../dataset/rbdcsr512_$dataset" -s"16" -d"$nr_dpu" -v"512" -t$file_type
            else 
                sudo LD_LIBRARY_PATH=$LD_LIBRARY_PATH ./bin/CSR_host -f"../dataset/ba_sparse_matrix_$dataset.txt" -s"16" -d"$nr_dpu" -v"512" -t$file_type
            fi
        done
    done
done    
