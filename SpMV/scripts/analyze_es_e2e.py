...
for file_index in range(num_files):
    ...
    if 'ES_256' in input_file:
        output_file = f'{output_dir}/es256-{a_to_value[a_value]}-e2e.txt'
    elif 'ES_512_e2e' in input_file:
        output_file = f'{output_dir}/es512-{a_to_value[a_value]}-e2e.txt'
    else:
        output_file = f'{output_dir}/output-{file_index + 1}-e2e.txt'
