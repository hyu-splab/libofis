...
for file_index in range(num_files):
    ...
    if 'OFIS_ig256' in input_file:
        output_file = f'{output_dir}/ofis256-ig-{a_to_value[a_value]}.txt'
    elif 'OFIS_ig512' in input_file:
        output_file = f'{output_dir}/ofis512-ig-{a_to_value[a_value]}.txt'
    elif 'OFIS_rank256' in input_file:
        output_file = f'{output_dir}/ofis256-rank-{a_to_value[a_value]}.txt'
    elif 'OFIS_rank512' in input_file:
        output_file = f'{output_dir}/ofis512-rank-{a_to_value[a_value]}.txt'
    else:
    ...
    with open(output_file, 'w') as outfile:
        ...
        for row in final_output[file_index * 5 : (file_index + 1) * 5]:
            ...
            reordered_row = [int(row[0]), "{:.4f}".format(row[2]), "{:.4f}".format(row[4]), 
                             "{:.4f}".format(row[3]), "{:.4f}".format(row[1]), "{:.4f}".format(row[5])]
