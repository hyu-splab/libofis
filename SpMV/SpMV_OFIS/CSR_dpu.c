...
__host volatile dpu_args_t DPU_INPUT_ARGS;
...
int main(){
    ...
    while(OFIS_dpu_state != 0){
        ...
        if(rows_per_tasklets != 0 && nr_rows != 0){
            ...
            // Type change
            // uint32_t* current_value = seqread_init(cache_value, (__mram_ptr void*)val_base, &sr_values);
            val_dt* current_value = seqread_init(cache_value, (__mram_ptr void*)val_base, &sr_values);


            // uint32_t acc; // result of current_row * input_vec
            // uint32_t* cache_input_vec = mem_alloc(8);
            // uint32_t* cache_output_vec = mem_alloc(8);
