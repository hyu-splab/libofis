        ...
        if (p_count >= csr_m->nr_part){ // Trigger DPU Binary to exit
            ...
            for(uint32_t i = 0; i < nr_finish_ig * 8; ++i){
                ...
                if(dpu_info[curr_p_count + i].nnz > max_nnz)
                    max_nnz = dpu_info[curr_p_count + i].nnz;
            }
...
int main(int argc, char** argv){
    ...
    dpu_info = (struct dpu_info_t*)malloc((csr_m->nr_part + 64) * sizeof(struct dpu_info_t));
    input_args = (dpu_args_t*)malloc((csr_m->nr_part + 64) * sizeof(dpu_args_t));
    ...
    for(uint32_t p_idx = csr_m->nr_part; p_idx < csr_m->nr_part + 64; ++p_idx){
        dpu_info[p_idx].nr_rows = 0;
        dpu_info[p_idx].start_row = 0;
        dpu_info[p_idx].start_idx = 0;
        dpu_info[p_idx].nnz = 0;
        dpu_info[p_idx].start_row_ptr = 0;
    
        input_args[p_idx].nr_rows = 0;
        input_args[p_idx].nr_cols = 0;
        input_args[p_idx].nnz = 0;
    }
    ...
    csr_m->values = (val_dt*)realloc(csr_m->values, (csr_m->nnz + max_nnz) * sizeof(val_dt));   
    output_vec = (val_dt*)calloc(csr_m->nr_part * max_nr_rows_dpu, sizeof(val_dt));
...
#pragma omp parallel for num_threads(64) shared (csr_m, result_vec, max_nr_rows_dpu) private(i,j)
    ...
    2: DPU Alloc & Load Time
    3: DPU Execution Time
    4: Merge Time
    5: Host Execution Time
    */
