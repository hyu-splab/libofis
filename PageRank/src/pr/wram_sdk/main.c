...
int main(int argc, char **argv) {
    ...
    DPU_ASSERT(dpu_load(set, "./bin/pr_wram_kernel", NULL));
    cur_Time(&ed);
    ...
    uint32_t max_edge;
    argument_pr* args = malloc(num_SGs * sizeof(argument));
    set_Argument_PR(args, &max_node, &max_icn, &max_edge);
    uint32_t p_count = 0;
    
    for(int i = 0; i < num_SGs; ++i){
        args[i].serv_rank = server_rank[i];
    }
    ...
    while(p_count < num_SGs){
        ...
        DPU_FOREACH(set, dpu, i) {
            DPU_ASSERT(dpu_prepare_xfer(dpu, args + p_count + i));
        }
        ...
        DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_TO_DPU, "ptr", 0, ptr_size, DPU_XFER_DEFAULT));
        cur_Time(&ed);
    ...
    FILE *fp = fopen("result/wram.csv", "a");
...
void initialize(int argc, char **argv) {
    if (argc == 2) {
        ...
        puts("3) ./bin/bc input_data num_SGs mode\n");
