...
int main() {
    ...
    if (tid == 0) {
        ...
        // rank = mem_alloc(MUL4_ALIGN8(arg.num_node));
    ...
    mram_read((__mram_ptr void const *)(ptr_base), cache_ptr, 8);
    ...
    for (int i = 0; i < ITER; ++i) {
        ...
        for(uint32_t node = start; node < end; ++node){
            ...
            uint32_t num_edge;
            if ((node << 2) > node_addr_aligned) {  // if odd'th node
                mram_read((__mram_ptr void const *)(ptr_base + node_addr_aligned), cache_ptr, 16);
                num_edge = cache_ptr[2] - cache_ptr[1];
                current_edge = seqread_init(cache_idx, (__mram_ptr void *)(idx_base + ((cache_ptr[1]) << 2)), &seq_idx);
            }
            else {
                mram_read((__mram_ptr void const *)(ptr_base + node_addr_aligned), cache_ptr, 8);
                num_edge = cache_ptr[1] - cache_ptr[0];
                current_edge = seqread_init(cache_idx, (__mram_ptr void *)(idx_base + ((cache_ptr[0]) << 2)), &seq_idx);
            }
            ...
            if (con_edge > 0) {
                if ((node << 2) > node_addr_aligned) current_edge = seqread_init(cache_idx, (__mram_ptr void *)(idx_base + ((cache_ptr[1]) << 2)), &seq_idx);
                else current_edge = seqread_init(cache_idx, (__mram_ptr void *)(idx_base + ((cache_ptr[0]) << 2)), &seq_idx);
